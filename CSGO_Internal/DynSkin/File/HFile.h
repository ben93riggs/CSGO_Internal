...
namespace File
{
    ...
    public:
	    ...
	    const std::string& const     GetPath()         { return _path; }
	    const vecFile& const         GetFile()         { return _file; }
        szPos								GetFileSize()     { return _file.size( ); }
	    const char& const            at( size_t p )     { return _file.at( p ); }


    private:

	    bool CompareBytes( char* pszData, const char* pszPattern )
        {
