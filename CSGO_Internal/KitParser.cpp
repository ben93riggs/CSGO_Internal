...
void InitializeKits()
{
	...
	auto sig_address = Utils::FindPattern(Utils::get_client_dll(), (PBYTE)XorStr("\xE8\x00\x00\x00\x00\xFF\x76\x0C\x8D\x48\x04\xE8"), XorStr("x????xxxxxxx"));

	while (!sig_address)
		sig_address = Utils::FindPattern(Utils::get_client_dll(), (PBYTE)XorStr("\xE8\x00\x00\x00\x00\xFF\x76\x0C\x8D\x48\x04\xE8"), XorStr("x????xxxxxxx"));
	...
	// Dump paint kits
	{
		...
		for (int i = 0; i <= map_head->last_element; ++i)
		{
			...
			const wchar_t* wide_name = LOCALIZE->Find(paint_kit->item_name.buffer + 1);
	...
	// Dump sticker kits
	{
		unsigned long ulTemp = Utils::FindPattern(Utils::get_client_dll(), (PBYTE)XorStr("\x53\x8D\x48\x04\xE8\x00\x00\x00\x00\x8B\x4D\x10"), XorStr("xxxxx????xxx"));
		
		while (!ulTemp)
			ulTemp = Utils::FindPattern(Utils::get_client_dll(), (PBYTE)XorStr("\x53\x8D\x48\x04\xE8\x00\x00\x00\x00\x8B\x4D\x10"), XorStr("xxxxx????xxx"));

		auto sticker_sig = ulTemp + 4;
		...
		for (int i = 0; i <= map_head->last_element; ++i)
		{
			...
			char sticker_name_if_valve_messed_up_their_translations[64];

			auto sticker_name_ptr = sticker_kit->item_name.buffer + 1;

			if (strstr(sticker_name_ptr, "StickerKit_dhw2014_dignitas"))
			{
				strcpy_s(sticker_name_if_valve_messed_up_their_translations, "StickerKit_dhw2014_teamdignitas");
				strcat_s(sticker_name_if_valve_messed_up_their_translations, sticker_name_ptr + 27);
				sticker_name_ptr = sticker_name_if_valve_messed_up_their_translations;
			}

			const wchar_t* wide_name = LOCALIZE->Find(sticker_name_ptr);
			auto name = converter.to_bytes(wide_name);
