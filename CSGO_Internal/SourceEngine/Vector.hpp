...
#define DEG2RAD( x  )  ( (float)(x) * (float)(M_PI_F / 180.f) )
	...
	public:
		...
		Vector& MultAdd(const Vector& vec, float scale)
		{
			x += vec.x * scale;
			y += vec.y * scale;
			z += vec.z * scale;
		...
		static vec_t DotProduct(const Vector& a, const Vector& b);
		// Dot product.
	...
	inline void VectorMultiply(const Vector &src, float scale, Vector &dst)
	{
		CHECK_VALID(a);
		Assert(IsFinite(b));
		dst.x = src.x * scale;
