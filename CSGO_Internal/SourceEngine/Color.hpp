...
#pragma warning(disable:4333 4244)
     ...
     public:
		 ...
		 Color(float rgb[3], float alpha)
		 {
			 SetColor(rgb[0] * 255, rgb[1] * 255, rgb[2] * 255, alpha * 255);
		 }
		 ...
		 //{
		 //	SetColor((uint8_t)(r * 255), (uint8_t)(g * 255), (uint8_t)(b * 255), (uint8_t)(a * 255));
		 //}
		 static Color FromHSB(float hsb[3])
		 {
			 float h = hsb[0] == 1.0f ? 0 : hsb[0] * 6.0f;
			 float f = h - (int)h;
			 ...
			 {
				 return Color(
					 static_cast<unsigned char>(hsb[2] * 255),
		 ...
		 float* ToArray() const
		 {
			 ...
			 *test[0] = (float)((int)_color[0] & 0xFF) / 255.0f;
			 *test[1] = (float)((int)_color[1] & 0xFF) / 255.0f;
			 *test[2] = (float)((int)_color[2] & 0xFF) / 255.0f;
			 *test[3] = 1.0f;
          ...
          DEFCOLOR_SRC(GreenYellow, 125, 255, 0);
